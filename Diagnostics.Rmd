...
testdata <- tibble(x = 1:10,
                   y = 1:10 + rnorm(10)) %>%
    add_row(x = 5.5, y = 12)

(lmod <- lm(y ~ x, testdata[1:10,]))
(lmod1 <- lm(y ~ x, testdata))

# the first example adds an outlier with a central predictor value
...
x_55 <- (predict(lmod1, tibble(x = 5.5, y = NA)) - lmod$coefficients[[1]]) / lmod$coefficients[[2]]

testdata %>%
    ggplot(aes(x = x, y = y)) +
    geom_point() +
    geom_point(data = tibble(x = c(5.5, 5.5), y = c(12,
                                                    predict(lmod1, newdata = tibble(x = 5.5, y = NA)))),
               aes(x = x, y = y), color = "mediumpurple", shape = 13, size = 5) +
    geom_segment(aes(x = 5.5,
                     y = 12,
                     xend = 5.5,
                     yend = predict(lmod1, newdata = tibble(x = 5.5, y = NA))),
                 lty = 2, alpha = 0.5, color = "firebrick1") +
    geom_segment(aes(x = 5.5,
                     y = predict(lmod, newdata = tibble(x = 5.5, y = NA)),
                     xend = 5.5,
                     yend = predict(lmod1, newdata = tibble(x = 5.5, y = NA))),
                 lty = 2, alpha = 0.5, color = "skyblue") +
    geom_segment(aes(x = 5.5,
                     y = predict(lmod1, tibble(x = 5.5, y = NA)),
                     xend = x_55,
                     yend = predict(lmod1, tibble(x = 5.5, y = NA))),
                 lty = 2, alpha = 0.5, color = "orange") +
