...
compare_summaries <- function(m1, m2) {
    ...
    bind_cols(m1, m2) %>%
        mutate(estimate = paste0("|", estimate_1, " - ",
                                 estimate_2, "| = ",
                                 abs(estimate_1 - estimate_2)),
               std.error = abs(std.error_1 - std.error_2),
               statistic = abs(statistic_1 - statistic_2),
               p.value = abs(p.value_1 - p.value_2)) %>%
