...
loo_params <- function(model) {
    model %>%
        tidy() %>%
        pull(1) %>%
        .[2:length(.)] %>%
        parse_quos(env = model$model) %>%
        map(., ~ loo_diff(model, .x))
}
...
lmod %>%
    ...
    pull(1) %>%
    .[2:length(.)] %>%
    parse_quos(env = lmod$model) -> quos_out

loo_diff(lmod, eval_tidy(expr = quo_get_expr(quos_out[[1]]),
                         env = quo_get_env(quos_out[[1]])))

quos_out %>%
    map(., ~ loo_diff(lmod, quo_get_expr(.x)))

lmod$model %>% as_tibble()

loo_diff(lmod, names(eval_tidy(parse_expr("dpi"),
                         data = as_tibble(lmod$model))))
...
sat %>%
    ...
    gen_all_partial_rp(response = "total")
```
