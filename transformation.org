...
byapr <- function(data, formula, cutoff = 0.05) {
    order <- 1
    repeat {
        new_form <- boost(formula, order)
        lmod <- lm(new_form, data)
        p_val <- tidy(lmod) %>% pluck(ncol(.), nrow(.))
        if (p_val > cutoff) break
        else order <- order + 1
    }
    lm(boost(formula, order - 1), data)
}

byapr(plt, y ~ x + I(x^2))
byapr(savings, frm)

tidy(lm(y ~ x + I(x^2) + I(x^3), plt))
...
byapr <- function(data, formula, cutoff = 0.05, order = 2) {
    build_up <- function(frm, order) {
        new_form <- boost(formula, order + 1)
        lmod <- lm(frm, data)
        p_val <- tidy(lmod) %>% pluck(ncol(.), nrow(.))
        if (p_val < cutoff) {
            cat("\n--------------\nOrder:", order, "\np-value:", p_val, "\nFormula:", deparse(frm), "\n--------------\n")
            build_up(new_form, order + 1)
        } else {
            lmod
        }
    }
    
    build_up(formula, order)
}



frm2 <- as.formula(y ~ x)
boost(boost(boost(frm2, 2), 3), 3)
