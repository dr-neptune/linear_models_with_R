...
# wooow. Now I just need to figure out how to reverse the build so we have order + 1 -> order
boost <- function(formula, order, expr = NULL) {
    term <- frm[[3]]
    if (order == 1) return(formula)
    else {
        formula[[3]] <- bquote(.(formula[[3]]) + I(.(term)^.(order)))
        boost(formula, (order - 1))
    }
}

form2 <- boost(frm, 5)
tidy(lm(form2, savings))
...
frm[[3]] %>% typeof()
frm[[3]]
frm[[3]] <- bquote(.(frm[[3]]) + I(ddpi^2))
frm[[3]] <- bquote(.(frm[[3]]) + I(.(frm[[3]][[2]])^3))

frm[[3]][[2]]

lmod <- tidy(lm(frm, savings))
lmod %>% pluck(ncol(.), nrow(.))



build_you_a_polynomial_regression <- function(data, formula, cutoff = 0.05, model = NULL) {
    boost <- function(formula, order) {
        formula[[3]] <- bquote(.(formula[[3]]) + I(.(formula[[3]][[2]])^.(order)))
    }
    
    if (model == NULL) {
        lmod <- lm(formula, data)
        p_val <- lmod %>% pluck(ncol(.), nrow(.))
        if (p_val <= cutoff) lmod
        else build_you_a_polynomial_regression(data, boost(formula, 2))
    } else {
