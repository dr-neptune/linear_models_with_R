...
#+BEGIN_SRC R :post round-tbl[:colnames yes](*this*)
(lmod <- lm(log(flux) ~ log(time), gammaray))
lmod %>% summary()
#+END_SRC

#+BEGIN_SRC R :file plot.svg :results graphics file
gammaray %>%
    ggplot(aes(x = log(time), y = log(flux))) +
    geom_point() +
    geom_abline(slope = lmod$coefficients[[2]],
                intercept = lmod$coefficients[[1]])
#+END_SRC
...
fform <- as.formula(brozek ~ age + weight + height +
               neck + chest + abdom + hip +
               thigh + knee + ankle + biceps +
               forearm + wrist)

# fit model 
lmod <- lm(fform, data = fat)
...
# fit Huber's model 
hmod <- rlm(fform, data = fat)
hmod %>% summary()
#+END_SRC

#+BEGIN_SRC R :file plot.svg :results graphics file
comp_tbl(list(lmod, hmod), c("least-squares", "Huber")) %>%
    plot_comp_tbl() +
    coord_flip()
#+END_SRC
...
Get model differences in coefficients 

#+BEGIN_SRC R :post round-tbl[:colnames yes](*this*)
get_weight_diff(lmod, hmod)
...
The coefficients are slightly different. The AIC BIC and log likelihood are the same for both models.
...
#+BEGIN_SRC R :post round-tbl[:colnames yes](*this*)
# get lowest weights
(lweights <- tibble("weights" = hmod$w,
                    "brozek" = fat$brozek,
                    "rnum" = 1:length(fat$brozek)) %>%
     arrange(weights))

# check for "unusual-ness"
lweights %>%
    slice(1:2)
...
They are both fairly close in height and weight, but they have very different brozek values (31.7 ~ 6.1)

#+BEGIN_SRC R :file plot.svg :results graphics file
fat %>%
    ggplot(aes(y = height, x = weight)) +
    geom_point()
#+END_SRC
...
We would expect our two outliers to be 

#+BEGIN_SRC R :post round-tbl[:colnames yes](*this*)
# outliers lower plot and far right
fat %>%
    filter(height < 35 | weight > 300) %>%
    glimpse()
#+END_SRC

These are not the same as the Huber method's values. This is probably because the Brozek values for these two people, while high, are not particularly strange given the other data. The values that Huber's method left out had very similar values for all the other predictors, but very different Brozek values. 
