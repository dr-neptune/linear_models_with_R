...
Now we can check the variance inflation factors

```{r}
# for the first variable
(summary(lm(x[,1] ~ x[,-1]))$r.squared -> vif_1)
1 / (1 - vif_1)
```

This is moderate in size. The VIF for orthogonal predictors is one. 

```{r}
# predict all VIFs in one go
vif(x)
```
